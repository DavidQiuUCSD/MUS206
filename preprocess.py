...
from datasets import blizzard, ljspeech, lakh
...
def preprocess_lakh(args):
  in_dir = os.path.join(args.base_dir, 'Lakh')
  out_dir = os.path.join(args.base_dir, args.output)
  os.makedirs(out_dir, exist_ok=True)
  metadata = lakh.build_from_path(in_dir, out_dir, args.num_workers, tqdm=tqdm)
  write_metadata(metadata, out_dir)
...
def main():
  ...
  parser.add_argument('--dataset', required=True, choices=['blizzard', 'ljspeech', 'lakh'])
